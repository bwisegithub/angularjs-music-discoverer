(function () {
	...
	angular.module('MusicDiscovererApp').directive('mdD3ForceGraph', function() { 
		...
		return { 
			...
			link: function(scope) {
				...
				var i;
				for (i=0; i<scope.mdassocobj.length; i+=1) {
				...
				// Determine height of graph by # of nodes to display, but no bigger than MAX_GRAPH_HEIGHT.
				var percOfMaxNodesAllowed = assocs.nodes.length / MAX_NUM_OF_NODES_TO_DISPLAY;
				var height = MAX_GRAPH_HEIGHT;
				if (percOfMaxNodesAllowed < 0.30) {
					// Don't need full height so overwrite height value with a percentage based one
					height = Math.min(MAX_GRAPH_HEIGHT, MAX_GRAPH_HEIGHT * (percOfMaxNodesAllowed + 0.50));
				} 
				...
				function processLinkedArr(linkedArray, musicianId) {
					...
					var k;
					for (k=0; k<linkedArray.length; k+=1) {
